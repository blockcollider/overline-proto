...
function encodeBcBlockForWireless (serializedBcBlock) {
  ...
  wirelessBcBlock.setHash(new Uint8Array(Buffer.from(bcBlock.getHash(), 'hex')))
  wirelessBcBlock.setPreviousHash(new Uint8Array(Buffer.from(bcBlock.getPreviousHash(), 'hex')))
  ...
  wirelessBcBlock.setDifficulty(difficulty.toArrayLike(Uint8Array, 'be', difficulty.byteLength()))
  wirelessBcBlock.setTimestamp(bcBlock.getTimestamp())
  wirelessBcBlock.setMerkleRoot(new Uint8Array(Buffer.from(bcBlock.getMerkleRoot(), 'hex')))
  wirelessBcBlock.setChainRoot(new Uint8Array(Buffer.from(bcBlock.getChainRoot(), 'hex')))
  const distance = new BN(bcBlock.getDistance())
  wirelessBcBlock.setDistance(distance.toArrayLike(Uint8Array, 'be', distance.byteLength()))
  const totalDistance = new BN(bcBlock.getTotalDistance())
  wirelessBcBlock.setTotalDistance(totalDistance.toArrayLike(Uint8Array, 'be', totalDistance.byteLength()))
  ...
  wirelessBcBlock.setEmblemChainFingerprintRoot(bcBlock.getEmblemChainFingerprintRoot())
  ...
  wirelessBcBlock.setBlockchainHeadersCount(bcBlock.getBlockchainHeadersCount())
  ...
  for (const chain of ['Btc', 'Eth', 'Lsk', 'Neo', 'Wav']) {
    ...
    for (const header of bcBlockList) {
      ...
      wirelessHeader.setBlockchainConfirmationsInParentCount(header.getBlockchainConfirmationsInParentCount())
  ...
  wirelessBcBlock.setBlockchainFingerprintsRoot(new Uint8Array(Buffer.from(bcBlock.getBlockchainFingerprintsRoot(), 'hex')))

  return wirelessBcBlock.serializeBinary()
}

function decodeWirelessBcBlockForBcnode (serializedWirelessBcBlock) {
  const wirelessBcBlock = WirelessBcBlock.deserializeBinary(serializedWirelessBcBlock)
  const bcBlock = new BcBlock()
  bcBlock.setHash(Buffer.from(wirelessBcBlock.getHash_asU8()).toString('hex'))
  bcBlock.setPreviousHash(Buffer.from(wirelessBcBlock.getPreviousHash_asU8()).toString('hex'))
  ...
  bcBlock.setMerkleRoot(Buffer.from(wirelessBcBlock.getMerkleRoot_asU8()).toString('hex'))
  bcBlock.setChainRoot(Buffer.from(wirelessBcBlock.getChainRoot_asU8()).toString('hex'))

  const wirelessDistance = new BN(wirelessBcBlock.getDistance_asU8(), 'be')
  bcBlock.setDistance(wirelessDistance.toString())
  const wirelessTotalDistance = new BN(wirelessBcBlock.getTotalDistance_asU8(), 'be')
  ...
  bcBlock.setEmblemChainFingerprintRoot(wirelessBcBlock.getEmblemChainFingerprintRoot())
  ...
  for (const wirelessTransaction of wirelessTxs) {
    const transaction = decodeWirelessTransactionForBcnode(wirelessTransaction, false)
  ...
  bcBlock.setBlockchainHeadersCount(wirelessBcBlock.getBlockchainHeadersCount())
  ...
  for (const chain of ['Btc', 'Eth', 'Lsk', 'Neo', 'Wav']) {
    ...
    for (const wirelessHeader of wirelessBcBlockList) {
      ...
      header.setBlockchainConfirmationsInParentCount(wirelessHeader.getBlockchainConfirmationsInParentCount())
  ...
  bcBlock.setBlockchainFingerprintsRoot(Buffer.from(wirelessBcBlock.getBlockchainFingerprintsRoot_asU8()).toString('hex'))

  return bcBlock.serializeBinary()
}

function encodeTransactionForWireless (inputTransaction, isInputSerialized = true) {
  let transaction
  ...
  wirelessTransaction.setHash(new Uint8Array(Buffer.from(transaction.getHash(), 'hex')))
  ...
  for (const input of inputs) {
    ...
    wirelessOutPoint.setHash(new Uint8Array(Buffer.from(outPoint.getHash(), 'hex')))
...
function decodeWirelessTransactionForBcnode (inputWirelessTransaction, isInputSerialized = true) {
  let wirelessTransaction
  if (isInputSerialized) {
    wirelessTransaction = WirelessTransaction.deserializeBinary(inputWirelessTransaction)
  } else {
  ...
  transaction.setHash(Buffer.from(wirelessTransaction.getHash_asU8()).toString('hex'))
  ...
  for (const wirelessInput of wirelessInputs) {
    ...
    outPoint.setHash(Buffer.from(wirelessOutPoint.getHash_asU8()).toString('hex'))
